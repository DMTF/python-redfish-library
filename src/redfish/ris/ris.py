...
class RisMonolithMember_v1_0_0(RisMonolithMemberBase):
    ...
    def __init__(self, restresp):
        """Init RisMonolithMember_v1_0_0 class."""
        self._resp = restresp
        self._patches = list()
        self._type = None
    ...
    type = property(_get_type, None)  # noqa: A003
    ...
    def load_from_dict(self, src):
        ...
        if "Type" in src:
            ...
            restreq = redfish.rest.v1.RestRequest(
                method="GET", path=src["OriginalUri"]
            )
    ...
    def _jsonpath_reducer(
        self, indict, breadcrumbs=None, outdict=OrderedDict()
    ):
        """JSON Path Reducer
    ...
    def update_member(self, member):
        ...
        for indices in range(len(self.types[member.maj_type]["Instances"])):
            ...
            if inst.resp.request.path == member.resp.request.path:
                ...
                self.types[member.maj_type]["Instances"][
                    indices
                ].patches.extend([patch for patch in inst.patches])
