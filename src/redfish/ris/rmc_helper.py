...
class UndefinedClientError(Exception):
    ...
    Usually triggered when user hasn't logged in
    """
...
class RmcClient(object):
    ...
    def __init__(
        self, url=None, username=None, password=None, sessionkey=None
    ):
    ...
    def set(self, path, args=None, body=None, headers=None):
        ...
        """
        return self.setup(path=path, args=args, body=body, headers=headers)

    def setup(self, path, args=None, body=None, headers=None):
        """Perform a PATCH request on path argument.
        ...
        return self._rest_client.patch(
            path=path, args=args, body=body, headers=headers
        )
...
class RmcFileCacheManager(RmcCacheManager):
    ...
    def __init__(self, rmc):
        """Init a RmcFileCacheManager class."""
                    ...
                    if url:
                        ...
                        if os.path.isfile(cachedir + "/" + index["href"]):
                            ...
                            for item in data:
                                if (
                                    "login" in item
                                    and "session_location" in item["login"]
                                ):
                                    loc = item["login"][
                                        "session_location"
            ...
            try:
                ...
                for client in clients_cache:
                    ...
                    for key in list(getdata.keys()):
                        ...
                        rmc_client._get_cache[
                            key
                        ] = redfish.rest.v1.StaticRestResponse(**getdata[key])
            ...
            for client in self._rmc._rmc_clients:
                ...
                clients_cache.append(
                    dict(
                        ...
                        monolith=client.monolith,
                        get=client._get_cache,
                    )
                )

                clientsfh = open(
                    "%s/%s" % (cachedir, index_map[client.get_base_url()]), "w"
                )
